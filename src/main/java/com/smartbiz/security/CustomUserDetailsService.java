...
public class CustomUserDetailsService implements UserDetailsService{
	...
	public UserDetails loadUserByUsername(String userName) throws UsernameNotFoundException {
		...
		return new SecurityUser(user.getEmailId(), user.getPassword(), 
				"A".equals(user.getStatus())?true:false, true, true, true, 
	...
	private List<GrantedAuthority> getGrantedAuthorities(User user){
		...
		logger.debug("Authorizing user as: "+user.getUserGroup().getGroupName().trim());
		authorities.add(new SimpleGrantedAuthority(user.getUserGroup().getGroupName().trim()));
